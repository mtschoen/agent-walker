    ...
    for (const fs::path& root : roots) {
        ...
        for (auto& slug_entry : fs::directory_iterator(root, ec)) {
            ...
            for (auto& file_entry : fs::directory_iterator(slug_entry.path(), ec)) {
                ...
                // Cached directory_entry method, not the fs::last_write_time(path)
                // free function: on Windows the entry caches the time from the
                // directory scan, so this avoids a fresh per-file stat (a
                // CreateFile/query/close round-trip per .jsonl — the dominant
                // discovery cost). Same for the subagent loop below.
