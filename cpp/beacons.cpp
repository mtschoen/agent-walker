...
std::vector<BeaconMatch> find_beacon_envelopes(std::string_view text) {
  ...
  while (pos < text.size()) {
    ...
    while (b_lo < c && std::isspace(static_cast<unsigned char>(text[b_lo])))
      ++b_lo;
    size_t b_hi = c;
    while (b_hi > b_lo &&
           std::isspace(static_cast<unsigned char>(text[b_hi - 1])))
      --b_hi;
...
void walk_assistant_entries(const fs::path &path, Callback &&cb) {
  ...
  while (pos < buffer.size()) {
      ...
      if (key == "timestamp") {
        ...
        for (auto msg_field : msg_obj) {
          ...
          if (msg_key == "role") {
            ...
            if (msg_field.value().get_array().get(arr) != sj::SUCCESS)
              continue;
            has_content = true;
            ...
            for (auto block_val : arr) {
              ...
              if (block_val.get_object().get(block) != sj::SUCCESS)
                continue;
              ...
              for (auto block_field : block) {
                std::string_view bk;
                if (block_field.unescaped_key().get(bk) != sj::SUCCESS)
                  continue;
                if (bk == "type") {
                  std::string_view tv;
    ...
    if (!is_assistant || !has_ts || !has_content)
      continue;
            ...
            if (ct == sj::ondemand::json_type::array) {
              ...
              for (auto block_val : arr) {
                ...
                for (auto block_field : block) {
                  ...
                  if (bk == "type") {
                    std::string_view tv;
                    if (block_field.value().get_string().get(tv) ==
                        sj::SUCCESS) {
                      if (tv == "text")
                        is_text_block = true;
                      else if (tv == "tool_result")
                        is_tool_result_block = true;
                    }
                  } else if (bk == "text") {
                    std::string_view tv;
                    if (block_field.value().get_string().get(tv) ==
                ...
                if (is_tool_result_block)
                  content_was_tool_result = true;
...
std::string format_number(double v) {
  if (v == std::floor(v) && std::isfinite(v) && v >= -1e15 && v <= 1e15) {
...
std::string serialize_beacon(const Beacon &b) {
  ...
  os << "{\"kind\":\"" << json_escape(b.kind) << "\""
     << ",\"eta_seconds\":" << format_number(b.eta_seconds) << ",\"summary\":\""
     << json_escape(b.summary) << "\"";
  if (b.has_drift) {
...
std::optional<LatestArgs>
parse_latest_args(const std::vector<std::string> &args, std::string &err) {
  LatestArgs out;
  ...
  for (size_t i = 0; i < args.size(); ++i) {
    ...
    if (flag == "--session-id") {
      ...
      if (!v)
        return std::nullopt;
      try {
        out.now_unix = std::stod(*v);
      } catch (...) {
        err = "--now: invalid number";
        return std::nullopt;
      }
...
std::optional<HistoryArgs>
parse_history_args(const std::vector<std::string> &args, std::string &err) {
  HistoryArgs out;
  for (size_t i = 0; i < args.size(); ++i) {
    const std::string &flag = args[i];
    auto need_value = [&](const std::string &f) -> std::optional<std::string> {
    ...
    if (flag == "--period") {
      ...
      if (!v)
        return std::nullopt;
      try {
        out.period_seconds = std::stoull(*v);
        out.have_period = true;
      } catch (...) {
        err = "--period: invalid integer";
        return std::nullopt;
      }
      ...
      if (!v)
        return std::nullopt;
      try {
        out.win_start_unix = std::stod(*v);
      } catch (...) {
        err = "--win-start: invalid number";
        return std::nullopt;
      }
    } else if (flag == "--projects-root") {
      auto v = need_value(flag);
...
std::optional<std::pair<Beacon, double>>
find_latest_in_path(const fs::path &path) {
  std::optional<std::pair<Beacon, double>> latest;
  walk_assistant_entries(
      path, [&](const std::string &text, const std::string &ts_str) {
        auto ts_opt = walker::parse_iso8601(ts_str);
        if (!ts_opt)
          return;
        double ts = *ts_opt;
...
std::optional<double>
compute_bias_factor(const std::vector<std::pair<double, double>> &pairs) {
  if (pairs.empty())
    return std::nullopt;
...
int run_history(const std::vector<std::string> &args) {
  ...
  walker::GroupMap groups =
      walker::discover_groups(roots, -std::numeric_limits<double>::infinity());
    ...
    while (true) {
      ...
      for (const auto &path : paths) {
        walk_entries_for_history(
            ...
            [&](const EventRow &row) { events.push_back(row); });
      }
      ...
      std::stable_sort(
          all_beacons.begin(), all_beacons.end(),
  ...
  std::vector<std::tuple<double, double, double>> pair_meta;
  size_t total = 0;
  for (const auto &l : per_thread)
    total += l.pairs.size();
  pairs.reserve(total);
  pair_meta.reserve(total);
  for (auto &l : per_thread) {
    pairs.insert(pairs.end(), std::make_move_iterator(l.pairs.begin()),
  ...
  os << "],\"session_count\":" << session_count
     << ",\"n_pairs\":" << pairs.size() << ",\"bias_factor\":";
  if (bias) {
